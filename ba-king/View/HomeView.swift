		...
		ZStack{
			...
			VStack(){
				...
				VStack(alignment: .leading, spacing: 18) {
					//Logo
					HStack(spacing:1){
						Text("B").font(.system(size: 50, weight: .heavy)).shadow(color:.black.opacity(0.2),radius: 0.5,x:0,y:3)
						Image("Logo").resizable().frame(width: 40, height: 40).ignoresSafeArea().shadow(color:.black.opacity(0.2),radius: 0.5,x:0,y:3)
						Text("KING").font(.system(size: 50, weight: .heavy)).shadow(color:.black.opacity(0.2),radius: 0.5,x:0,y:3)
						Spacer()
					}.padding(.leading,30)
					
					VStack(alignment:.leading,spacing:1){
						Text("Choose the recipe you're doing!").font(.system(size: 15, weight: .bold)).shadow(color:.black.opacity(0.2),radius: 0.5,x:0,y:3)
						Text("Scroll and tap to continue").font(.system(size: 10, weight: .light)).shadow(color:.black.opacity(0.2),radius: 0.5,x:0,y:3)
						Spacer()
					}.padding(.leading,30)
				}.padding(.top,50)
